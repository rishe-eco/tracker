/** Monitoring Lab `v1` — assembles locale-invariant specs with a locale surface. */

import type { MonitoringPack, Locale } from "../types";
...
export function buildMonitoringPack(locale: Locale): MonitoringPack {
  ...
  const items = ITEM_SPECS.map((spec) => {
    ...
    if (!itemSurface) {
      throw new Error(`Missing ${locale} surface for monitoring item "${spec.itemId}". Every itemId must exist in every locale.`);
    }
  ...
  return {
    ...
    modules: MODULE_ORDER.map((key) => {
      ...
      if (!mod) throw new Error(`Missing ${locale} surface for monitoring module "${key}".`);
      return mod;
    }),
    items,
  };
...
export { CONTENT_VERSION, ITEM_SPECS, ITEM_SPEC_BY_ID, MODULE_ORDER, COUNTERMEASURE_OPTIONS, COUNTERMEASURE_HAS_TRIGGER } from "./spec";
