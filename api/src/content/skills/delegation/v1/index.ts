/** Delegation Lab `v1` — assembles locale-invariant specs with a locale surface. */

import type { DelegationPack, Locale } from "../types";
...
export function buildDelegationPack(locale: Locale): DelegationPack {
  ...
  const items = ITEM_SPECS.map((spec) => {
    ...
    if (!itemSurface) {
      throw new Error(`Missing ${locale} surface for delegation item "${spec.itemId}". Every itemId must exist in every locale.`);
    }
  ...
  return {
    ...
    modules: MODULE_ORDER.map((key) => {
      ...
      if (!mod) throw new Error(`Missing ${locale} surface for delegation module "${key}".`);
      return mod;
    }),
    items,
  };
